# Build TeenAstro MSI: MainUnit + SHC emulators; optionally Firmware Uploader and TeenAstro App (full package).
# For Uploader-only or App-only MSI, use TeenAstroUploader\installer\build.ps1 or teenastro_app\installer\build.ps1.
# Requires: PlatformIO (pio), WiX Toolset 3. Optional: MSBuild (uploader), Flutter (app), MinGW (emulator).
#
# Run from repo root: .\TeenAstroEmulator\installer\build.ps1
# Or from this folder: .\build.ps1 (script detects repo root)
#
# Switches:
#   -EmulatorsOnly   Build and install only the emulators (MainUnit + SHC). No Uploader, no App.
...
# When -SkipApp or -EmulatorsOnly, clear stage so we don't harvest leftover files.
if ($SkipApp -or $EmulatorsOnly) {
    Get-ChildItem $StageDir -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
}

# Emulators: stage all build output (exes + all DLLs: SDL2, MinGW runtime, etc.) so MSI has everything (full or EmulatorsOnly)
if ($true) {
    $emuStaged = $false
...
# Uploader: single exe when full build (not EmulatorsOnly)
...
# Flutter app -- copy all files flat into stage (exe + DLLs + data/) when full build and not -SkipApp
...
try {
    ...
    # EmulatorsOnly: use relative StageDir so candle/light do not mis-parse "C:\"
    $stageDirArg = if ($EmulatorsOnly) { ".stage" } else { $StageDir }
    ...
    # EmulatorsOnly: use relative -b and -out so light.exe does not mis-parse "C:\"
    $bindPath = if ($EmulatorsOnly) { "." } else { $StageDir }
    $lightOut = if ($EmulatorsOnly) { ".out\TeenAstroEmulator.msi" } else { $msi }
