...
byte PushToHor(Coord_HO HO_T, PoleSide preferedPoleSide, float* deltaA1, float* deltaA2)
{
  ...
  if (HO_T.Alt() < mount.minAlt * DEG_TO_RAD) return ERRGOTO_BELOWHORIZON;   // fail, below min altitude
  if (HO_T.Alt() > mount.maxAlt * DEG_TO_RAD) return ERRGOTO_ABOVEOVERHEAD;   // fail, above max altitude
  ...
  Axis1_target = axis1_target / mount.geoA1.stepsPerDegree;
  Axis2_target = axis2_target / mount.geoA2.stepsPerDegree;
  *deltaA1 = mount.encoderA1.deltaTarget(Axis1_target);
  *deltaA2 = mount.encoderA2.deltaTarget(Axis2_target);
  return ERRGOTO_NONE;
}
