  ...
  const double minInterval, const double maxInterval)
{
  ...
  volatile double sumRateA = mount.sideralTracking ? fabs(tmp_guideRateA + staA->CurrentTrackingRate) : fabs(tmp_guideRateA);
...
static void UpdateIntervalTrackingGuiding1()
{
  ...
  UpdateIntervalTrackingGuiding(&mount.guideA1, &mount.staA1, &mount.geoA1, &mount.staA2, tmp_guideRateA1, mount.minInterval1, mount.maxInterval1);
}

static void UpdateIntervalTrackingGuiding2()
{
  static volatile double tmp_guideRateA2 = 0;
  UpdateIntervalTrackingGuiding(&mount.guideA2, &mount.staA2, &mount.geoA2, &mount.staA1, tmp_guideRateA2, mount.minInterval2, mount.maxInterval2);
}
...
static void BacklashAndApplyInterval1()
{
  ...
  volatile double thisIntervalAxis1 = mount.staA1.interval_Step_Cur;
  BacklashComp(&mount.guideA1, &mount.staA1, thisIntervalAxis1, wasInbacklashAxis1);
...
static void BacklashAndApplyInterval2()
{
  ...
  volatile double thisIntervalAxis2 = mount.staA2.interval_Step_Cur;
  BacklashComp(&mount.guideA2, &mount.staA2, thisIntervalAxis2, wasInbacklashAxis2);
...
{ 
  ...
  if (!mount.movingTo)
  {
    ...
    if (!mount.guideA1.isBusy() && !mount.guideA2.isBusy())
    {
      mount.GuidingState = Guiding::GuidingOFF;
    }
  }
...
ISR(TIMER3_COMPA_vect)
{
  ...
  if (clearAxis1)
  {
    ...
    if (mount.staA1.deltaTarget != 0 || mount.staA1.backlash_correcting)
    {
      // Move the RA stepper to the target
      mount.staA1.dir = 0 < mount.staA1.deltaTarget;
      // Direction control
      if (mount.motorA1.reverse ^ Axis1Reverse)
      {
        digitalWriteFast(Axis1DirPin, !mount.staA1.dir);
      }
      else
      {
        digitalWriteFast(Axis1DirPin, mount.staA1.dir);
      ...
      mount.staA1.move(!isGuidingStar());
      takeStepAxis1 = true;
    }
    clearAxis1 = false;
  }
...
ISR(TIMER4_COMPA_vect)
{
  ...
  if (clearAxis2)
  {
    ...
    if (mount.staA2.deltaTarget != 0 || mount.staA2.backlash_correcting)
    {
      // move the Dec stepper to the target
      // telescope normally starts on the EAST side of the pier looking at the WEST sky
      mount.staA2.dir = 0 < mount.staA2.deltaTarget;
      // Direction control
      if (mount.motorA2.reverse ^ Axis2Reverse)
      {
        digitalWriteFast(Axis2DirPin, !mount.staA2.dir);
      }
      else
      {
        digitalWriteFast(Axis2DirPin, mount.staA2.dir);
      ...
      mount.staA2.move(!isGuidingStar());
      takeStepAxis2 = true;
    }
    clearAxis2 = false;
  }
