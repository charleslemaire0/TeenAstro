...
- In source, display strings (e.g. `FirmwareNumber`, `FirmwareVersion`, `SHCFirmwareVersionPatch`) may still use three segments (e.g. `1.6.0`) for compatibility, but the **release version** that users see and download is **1.6**. When bumping versions, **increment each product’s own semver by one step** (do not force every component to the same patch number).
