...
"&nbsp;&nbsp;&nbsp;<button class='bbh' type='submit' name=\"al\" value=\"n\">Accept</button></form></div>";
#endif

const char html_controlParkHome[] PROGMEM =
"<div class='panel'>"
"<div class='panel-title'>Park &amp; Home</div>"
...
void TeenAstroWifi::handleControl()
{
  ...
  if (ta_MountStatus.hasFocuser())
  {
    ...
    data += "<div class='panel-title' style='display:flex;justify-content:space-between'>"
            "<span>Focuser</span><span id='focuserpos'>?</span></div>";
...
void TeenAstroWifi::processControlGet()
{
  ...
  if (v != "")
  {
    ...
    else if (v == "qh") s_client->homeGoto();
    else if (v == "pr") s_client->parkReset();
    else if (v == "ps") s_client->setPark();
    else if (v == "pk") s_client->park();
    else if (v == "pu") s_client->unpark();
