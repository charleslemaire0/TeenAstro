...
template = env.get_template("ngc_template.jinja")
...
class ngcObject():
  def __init__(self, dso):
    ...
    if (dso.parallax != None):
        print (dso.name, dso.parallax)
    ...
    if (ch in 'ABCD'):
        ...
        self.subId = ' ABCD'.index(ch)   # set subId to index of last character (index 0 means no subId)
...
# Get NGC objects
dsoList = ongc.listObjects(catalog = 'NGC', uptovmag = 16.0)
...
with open("renders/ta_ngc_select_c.h", 'w') as f:
    print (template.render(numObjects=len(ngcList), objects=ngcList), file=f)