...
const char      index_page[] = "GET /index.htm";
bool            index_page_found;
byte            index_page_count;
const char      settings_page[] = "GET /settings.htm";
bool            settings_page_found;
byte            settings_page_count;
const char      control_page[] = "GET /control.htm";
bool            control_page_found;
byte            control_page_count;
const char      guide_page[] = "GET /guide.htm";
bool            guide_page_found;
byte            guide_page_count;
const char      pec_page[] = "GET /pec.htm";
bool            pec_page_found;
byte            pec_page_count;
const char      config_page[] = "GET /config.htm";
bool            config_page_found;
byte            config_page_count;
const char      guide_ajax[] = "GET /guide.txt";
bool            guide_ajax_found;
byte            guide_ajax_count;

void reset_page_requests()
{
  index_page_found = false;
  index_page_count = 0;
  settings_page_found = false;
  settings_page_count = 0;
  control_page_found = false;
  control_page_count = 0;
  guide_page_found = false;
  guide_page_count = 0;
  pec_page_found = false;
  pec_page_count = 0;
  config_page_found = false;
  config_page_count = 0;
  guide_ajax_found = false;
  guide_ajax_count = 0;
  get_check = false;
  get_val = false;
  get_name = false;
  ...
  if (www_client)
  {
    if (www_client.connected())
    {
      if (www_client.available() && (!responseStarted))
      {
        ...
        if (!index_page_found)
        {
          if (c == index_page[index_page_count])
            index_page_count++;
          else
            index_page_count = 0;
          if (index_page_count == 14)
          {
            index_page_found = true;
            get_check = true;
          }
        }

        if (!settings_page_found)
        {
          if (c == settings_page[settings_page_count])
            settings_page_count++;
          else
            settings_page_count = 0;
          if (settings_page_count == 17)
          {
        ...
        if (!control_page_found)
        {
          if (c == control_page[control_page_count])
            control_page_count++;
          else
            control_page_count = 0;
          if (control_page_count == 16)
          {
            control_page_found = true;
            get_check = true;
          }
        }
        ...
        if (!guide_page_found)
        {
          if (c == guide_page[guide_page_count])
            guide_page_count++;
          else
            guide_page_count = 0;
          if (guide_page_count == 14)
          {
            guide_page_found = true;
            get_check = true;
          }
        }
        ...
        if (!pec_page_found)
        {
          if (c == pec_page[pec_page_count])
            pec_page_count++;
          else
            pec_page_count = 0;
          if (pec_page_count == 12)
          {
            pec_page_found = true;
            get_check = true;
          }
        ...
        if (!config_page_found)
        {
          if (c == config_page[config_page_count])
            config_page_count++;
          else
            config_page_count = 0;
          if (config_page_count == 15)
          {
        ...
        if (!guide_ajax_found)
        {
          if (c == guide_ajax[guide_ajax_count])
            guide_ajax_count++;
          else
            guide_ajax_count = 0;
          if (guide_ajax_count == 14)
          {
            guide_ajax_found = true;
            get_check = true;
          }
        }
      ...
      if (responseStarted)
      {
        if (www_xmit_buffer_pos == 0)
        {
          if (index_page_found)
            index_html_page();
          else if (settings_page_found)
            settings_html_page();
          else if (control_page_found)
            control_html_page();
          else if (guide_page_found)
            guide_html_page();
          else if (pec_page_found)
            pec_html_page();
          else if (config_page_found)
            config_html_page();
          else if (guide_ajax_found)
            guide_ajax_page();
          else
            index_html_page();
        }
    ...
    if (((clientNeedsToClose && (millis() - responseFinish_ms > 100)) ||
      (millis() - transactionStart_ms > 5000)) ||
...
boolean www_send()
{
  ...
  for (int l = 0; l < 48; l++)
  {
    ...
    buf[l + 1] = 0;
    buf[l] = c;
    if ((c == 0) || (www_xmit_buffer_send_pos > www_xmit_buffer_size - 2))
    {
      buffer_empty = true;
      break;
    }
...
void Ethernet_get()
{
  ...
  {
    if ((get_vals[0] == 'v') && (get_vals[1] == 's') && (get_vals[2] == 0))
      maxRate = MaxRate * 32L;
    if ((get_vals[0] == 's') && (get_vals[1] == 0))
      maxRate = MaxRate * 24L;
    if ((get_vals[0] == 'n') && (get_vals[1] == 0))
      maxRate = MaxRate * 16L;
    if ((get_vals[0] == 'f') && (get_vals[1] == 0))
      maxRate = MaxRate * 11L;
    if ((get_vals[0] == 'v') && (get_vals[1] == 'f') && (get_vals[2] == 0))
      maxRate = MaxRate * 8L;
  ...
  {
    if ((atoi2(get_vals, &i)) && ((i >= -180) && (i <= 180)))
    {
      get_temp_float = i;
    }
  }
  ...
  if ((get_names[0] == 'g') && (get_names[1] == '2'))
  {
    if ((atoi2(get_vals, &i)) && ((i >= 0) && (i <= 60)))
    {
      ...
      longitude = get_temp_float + ((double)i) / 60.0;
  ...
  if ((get_names[0] == 'd') && (get_names[1] == 'y'))
  {
    if ((atoi2(get_vals, &i)) && ((i >= 2016) && (i <= 9999)))
    {
      ...
      char    temp[10];
      sprintf(temp, "%02d/%02d/%02d", get_temp_month, get_temp_day,
        get_temp_year);
  ...
  if ((get_names[0] == 't') && (get_names[1] == 's'))
  {
    if ((atoi2(get_vals, &i)) && ((i >= 0) && (i <= 59)))
    {
      ...
      char    temp[10];
      sprintf(temp, "%02d:%02d:%02d", get_temp_hour, get_temp_minute,
        get_temp_second);
      i = highPrecision;
      highPrecision = true;
  ...
  {
    if ((get_vals[0] == 'o') && (get_vals[1] == 'n') && (get_vals[2] == 0))
      trackingState = TrackingON;
    if ((get_vals[0] == 'o') && (get_vals[1] == 'f') && (get_vals[2] == 'f') &&
      (get_vals[3] == 0))
      trackingState = TrackingOFF;
    if (((get_vals[0] == 'f') || (get_vals[0] == '-') || (get_vals[0] == 'r')
      ) && (get_vals[1] == 0))
    {
    ...
    if ((get_vals[0] == 's') && (get_vals[1] == 0))
      SetTrackingRate(default_tracking_rate); // sidereal
    if ((get_vals[0] == 'l') && (get_vals[1] == 0))
      SetTrackingRate(0.96236513150);         // lunar
    if ((get_vals[0] == 'h') && (get_vals[1] == 0))
      SetTrackingRate(0.99726956632);         // solar
  }

  // Refraction Rate Tracking control
  if ((get_names[0] == 'r') && (get_names[1] == 'r'))
  {
    if ((get_vals[0] == 'o') && (get_vals[1] == 't') && (get_vals[2] == 'k') &&
      (get_vals[3] == 0))
    {
      ...
      if (get_vals[2] == 0)
      {
        ...
        {   // start guide
          if ((get_vals[0] == 'n') || (get_vals[0] == 's'))
          {
            ...
            if ((guideDirAxis2) && (get_vals[0] != guideDirAxis2) &&
              (fabs(guideTimerRateAxis2) > 2))
            {
              guideDirAxis2 = 'b';
            }
            else
            {
              ...
              guideDurationDec = -1;
              cli();
              guideTimerRateAxis2 = guideTimerBaseRate;
              sei();
            }

            quietReply = true;
          }
          else if ((get_vals[0] == 'e') || (get_vals[0] == 'w'))
          {
            // block user from changing direction at high rates, just stop the guide instead
            if ((guideDirAxis1) && (get_vals[0] != guideDirAxis1) &&
              (fabs(guideTimerRateAxis1) > 2))
            {
              guideDirAxis1 = 'b';
            }
            else
        ...
        else if ((get_vals[1] == 'y') && (get_vals[0] == 's'))
        {               // sync
  ...
  {
    if ((get_vals[0] == 'p') && (get_vals[1] == 'l') && (get_vals[2] == 0))
    {
      if (pecRecorded) pecStatus = ReadyPlayPEC;
      EEPROM.update(EE_pecStatus, pecStatus);
    }                       // play

    if ((get_vals[0] == 's') && (get_vals[1] == 't') && (get_vals[2] == 0))
    {
      pecStatus = IgnorePEC;
      EEPROM.update(EE_pecStatus, pecStatus);
    };
    ...
    // stop
    if ((get_vals[0] == 'r') && (get_vals[1] == 'e') && (get_vals[2] == 0))
    {
      pecStatus = ReadyRecordPEC;
      EEPROM.update(EE_pecStatus, IgnorePEC);
    };

    // record
    if ((get_vals[0] == 'c') && (get_vals[1] == 'l') && (get_vals[2] == 0))
    {                       // clear
    ...
    if ((get_vals[0] == 'w') && (get_vals[1] == 'r') && (get_vals[2] == 0))
    {                       // write to eeprom
      pecRecorded = true;
      EEPROM.update(EE_pecRecorded, pecRecorded);
      EEPROM_writeLong(EE_wormSensePos, wormSensePos);
...
#define Axis2   "Dec"
#endif
const char  html_index1[] PROGMEM = "<div class=\"t\"><table width=\"100%\"><tr><td><b>"FirmwareName " "
FirmwareNumber;
...
const char  html_indexTrue[] PROGMEM = "Absolute Position: "Axis1
"=<font class=\"c\">%ld</font> steps, "Axis2
"=<font class=\"c\">%ld</font> steps<br />";
...
const char  html_indexPosition[] PROGMEM = "Instrument Coordinates: "Axis1
"=<font class=\"c\">%s</font>, "Axis2 "=<font class=\"c\">%s</font><br />";
const char  html_indexTarget[] PROGMEM = "Target Coordinates: "Axis1
"=<font class=\"c\">%s</font>, "Axis2 "=<font class=\"c\">%s</font><br />";
const char  html_indexAz1[] PROGMEM = "Az1: "Axis1
"=<font class=\"c\">%s</font>, "Axis2 "=<font class=\"c\">%s</font><br />";
const char  html_indexAz2[] PROGMEM = "Az2: "Axis1
"=<font class=\"c\">%s</font>, "Axis2 "=<font class=\"c\">%s</font><br />";
...
const char  html_indexFault[] PROGMEM = "Stepper Driver: "Axis1 " axis %s, "
Axis2 " axis %s<br /><br />";
...
void index_html_page()
{
  ...
  if (html_page_step == ++stp)
  {
    ...
    highPrecision = i;
    strcpy_P(temp1, html_index2b);
    sprintf(temp, temp1, temp2, temp3);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    strcpy_P(temp1, html_index3);
    sprintf(temp, temp1, i, i1, i2);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    strcpy_P(temp1, html_index4);
    sprintf(temp, temp1, temp2);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    sei();
    strcpy_P(temp1, html_indexTrue);
    sprintf(temp, temp1, a1, a2);
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_indexIndex);
    sprintf(temp, temp1, indexAxis1Steps, indexAxis2Steps);
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_indexCorIdx);
    sprintf(temp, temp1, (long)(indexAxis1 * 3600.0),
      (long)(indexAxis2 * 3600.0));
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_indexCorPole);
    sprintf(temp, temp1, (long)(altCor * 3600.0), (long)(azmCor * 3600.0));
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_indexCorOrtho);
    sprintf(temp, temp1, (long)(doCor * 3600.0), (long)(pdCor * 3600.0));
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    highPrecision = i;
    strcpy_P(temp1, html_indexRateDeltas);
    sprintf(temp, temp1, temp2, temp3);
  }
  ...
  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_indexCorPolar);
    sprintf(temp, temp1, (long)(altCor * 3600.0), (long)(azmCor * 3600.0));
  }
#endif
#endif
  if (html_page_step == ++stp)
  {
    ...
    sei();
#ifdef MOUNT_TYPE_ALTAZM
    double  ha = (double)h / StepsPerDegreeAxis1;
    doubleToDms(temp2, &ha, true, true);
#else
    double  ha = (double)h / (StepsPerDegreeAxis1 * 15.0);
    doubleToHms(temp2, &ha);

    double  ra = timeRange(rtk.rtk.LST() - ha);
    doubleToHms(temp3, &ra);
    sprintf(temp2, "%s</font> (RA=<font class=\"c\">%s</font>)", temp2,
      temp3);
#endif
    ...
    strcpy_P(temp1, html_indexPosition);
    sprintf(temp, temp1, temp2, temp3);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    double  dec = (double)d / StepsPerDegreeAxis2;
    doubleToDms(temp3, &dec, false, true);
    highPrecision = i;

    strcpy_P(temp1, html_indexTarget);
    sprintf(temp, temp1, temp2, temp3);
  }

  if (html_page_step == ++stp)
  {
    if ((pierSide == PierSideFlipWE1) || (pierSide == PierSideFlipWE2) ||
      (pierSide == PierSideFlipWE3))
      strcpy(temp2, "Meridian Flip, West to East");
    else if ((pierSide == PierSideFlipEW1) || (pierSide == PierSideFlipEW2) ||
      (pierSide == PierSideFlipEW3))
      strcpy(temp2, "Meridian Flip, East to West");
    else if (pierSide == PierSideWest)
      strcpy(temp2, "West");
    else if (pierSide == PierSideEast)
      strcpy(temp2, "East");
    else if (pierSide == PierSideNone)
      strcpy(temp2, "None");
    else
      strcpy(temp2, "Unknown");
    if (meridianFlip == MeridianFlipNever)
      strcpy(temp3, "Disabled");
    else
      strcpy(temp3, "Enabled");
    strcpy_P(temp1, html_indexPier);
    sprintf(temp, temp1, temp2, temp3);
  }

  if (html_page_step == ++stp)
  {
    if (trackingState == TrackingOFF) strcpy(temp2, "Off");
    if (trackingState == TrackingON) strcpy(temp2, "On");
    ...
    if (temp3[strlen(temp3) - 2] == ',')
    {
      temp3[strlen(temp3) - 2] = ')';
      temp3[strlen(temp3) - 1] = 0;
    }
    else
      strcpy(temp3, "");

    strcpy_P(temp1, html_index8);
    sprintf(temp, temp1, temp2, temp3);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    strcpy_P(temp1, html_index9);
    sprintf(temp, temp1, temp2, (worst_loop_time * 100L) / 9970L);
    worst_loop_time = 0;
  }
  ...
  if (html_page_step == ++stp)
  {
    if (lastError == ERR_NONE)
      ...
      strcpy(temp2, "Motor Fault");
    else if (lastError == ERR_ALT)
      strcpy(temp2, "Altitude Min/Max");
    else if (lastError == ERR_LIMIT_SENSE)
      strcpy(temp2, "Limit Sense");
    else if (lastError == ERR_DEC)
      strcpy(temp2, "Dec Limit Exceeded");
    else if (lastError == ERR_AZM)
      strcpy(temp2, "Azm Limit Exceeded");
    else if (lastError == ERR_UNDER_POLE)
      strcpy(temp2, "Under Pole Limit Exceeded");
    else if (lastError == ERR_MERIDIAN)
      strcpy(temp2, "Meridian Limit (W) Exceeded");
    else if (lastError == ERR_SYNC)
      strcpy(temp2, "Sync. ignored >30&deg;");
    strcpy_P(temp1, html_index10);
    sprintf(temp, temp1, temp2);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    sprintf(temp, temp1, faultAxis1 ? "<font class=\"c\">FAULT</font>" :
      "<font class=\"g\">Ok</font>",
      faultAxis2 ? "<font class=\"c\">FAULT</font>" :
      "<font class=\"g\">Ok</font>");
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_indexMaxRate);
    sprintf(temp, temp1, maxRate / 16L, (long)MaxRate);
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_index11);
    sprintf(temp, temp1, (worst_loop_time * 100L) / 9970L);
    worst_loop_time = 0;
  }
...
const char  html_settings1[] PROGMEM = "<div class=\"t\"><table width=\"100%\"><tr><td><b>"FirmwareName " "
FirmwareNumber;
...
const char  html_settings4[] PROGMEM = "Maximum Slew Speed: <form method=\"get\" action=\"/settings.htm\"><button name=\"ss\" value=\"vs\" type=\"submit\">Very Slow (0.5x)</button><button name=\"ss\" value=\"s\" type=\"submit\">Slow (0.75x)</button>";
const char  html_settings5[] PROGMEM = "<button name=\"ss\" value=\"n\" type=\"submit\">Normal (1x) </button><button name=\"ss\" value=\"f\" type=\"submit\">Fast (1.5x) </button><button name=\"ss\" value=\"vf\" type=\"submit\">Very Fast (2x) </button></form><br />\r\n";
const char  html_settings7[] PROGMEM = "Backlash: <br /><form method=\"get\" action=\"/settings.htm\"> <input value=\"%d\" type=\"number\" name=\"b1\" min=\"0\" max=\"999\"><button type=\"submit\">Upload</button> ("
Axis1a ", in arc-seconds 0 to 999)</form>\r\n";
const char  html_settings8[] PROGMEM = "<form method=\"get\" action=\"/settings.htm\"> <input value=\"%d\" type=\"number\" name=\"b2\" min=\"0\" max=\"999\"><button type=\"submit\">Upload</button> ("
Axis2 ", in arc-seconds 0 to 999)</form><br />\r\n";
const char  html_settings9[] PROGMEM = "Limits: <br /><form method=\"get\" action=\"/settings.htm\"> <input value=\"%d\" type=\"number\" name=\"hl\" min=\"-30\" max=\"30\"><button type=\"submit\">Upload</button> (Horizon, in degrees +/- 30)</form>\r\n";
const char  html_settings10[] PROGMEM = "<form method=\"get\" action=\"/settings.htm\"> <input value=\"%d\" type=\"number\" name=\"ol\" min=\"60\" max=\"90\"><button type=\"submit\">Upload</button> (Overhead, in degrees 60 to 90)</form><br />\r\n";
const char  html_settings11[] PROGMEM = "Location: <br /><form method=\"get\" action=\"/settings.htm\"> <input value=\"%s\" type=\"number\" name=\"g1\" min=\"-180\" max=\"180\">&nbsp;&deg;&nbsp;";
const char  html_settings12[] PROGMEM = " <input value=\"%s\" type=\"number\" name=\"g2\" min=\"0\" max=\"60\">&nbsp;'&nbsp;&nbsp;<button type=\"submit\">Upload</button> (Longitude, in deg. and min. +/- 180)</form>\r\n";
const char  html_settings13[] PROGMEM = "<form method=\"get\" action=\"/settings.htm\"> <input value=\"%s\" type=\"number\" name=\"t1\" min=\"-90\" max=\"90\">&nbsp;&deg;&nbsp;";
const char  html_settings14[] PROGMEM = " <input value=\"%s\" type=\"number\" name=\"t2\" min=\"0\" max=\"60\">&nbsp;'&nbsp;&nbsp;<button type=\"submit\">Upload</button> (Latitude, in deg. and min. +/- 90)</form>\r\n";
const char  html_settings15[] PROGMEM = "<form method=\"get\" action=\"/settings.htm\"> <input value=\"%s\" type=\"number\" name=\"u1\" min=\"-12\" max=\"14\">&nbsp;hrs&nbsp;";
const char  html_settings16[] PROGMEM = " <select name=\"u2\"><option value=\"0\" %s>00</option><option value=\"30\" %s>30</option><option value=\"45\" %s>45</option></select>&nbsp;min.&nbsp;<button type=\"submit\">Upload</button> (UTC Offset, in hours -12 to +14)</form>\r\n";
...
void settings_html_page()
{
  ...
  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_settings7);
    i = (int)round(((double)backlashAxis1 * 3600.0) / StepsPerDegreeAxis1);
    if (i < 0) i = 0;
    if (i > 999) i = 999;
    sprintf(temp, temp1, i);
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_settings8);
    i = (int)round(((double)backlashAxis2 * 3600.0) / StepsPerDegreeAxis2);
    if (i < 0) i = 0;
    if (i > 999) i = 999;
    sprintf(temp, temp1, i);
  }
  ...
  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_settings9);
    sprintf(temp, temp1, minAlt);
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_settings10);
    sprintf(temp, temp1, maxAlt);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    highPrecision = i;
    strcpy_P(temp1, html_settings11);
    sprintf(temp, temp1, temp2);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    doubleToDms(temp2, &longitude, true, true);
    highPrecision = i;
    strcpy_P(temp1, html_settings12);
    sprintf(temp, temp1, (char *)&temp2[5]);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    if (temp2[0] == '+') temp2[0] = '0';    // remove +
    highPrecision = i;
    strcpy_P(temp1, html_settings13);
    sprintf(temp, temp1, temp2);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    doubleToDms(temp2, &latitude, false, true);
    highPrecision = i;
    strcpy_P(temp1, html_settings14);
    sprintf(temp, temp1, (char *)&temp2[4]);
  }
  ...
  if (html_page_step == ++stp)
  {
    ...
    temp2[3] = 0;   // deg. part only
    if (temp2[0] == '+') temp2[0] = '0';    // remove +
    strcpy_P(temp1, html_settings15);
    sprintf(temp, temp1, temp2);
  }
...
const char  html_control1[] PROGMEM = "<div class=\"t\"><table width=\"100%\"><tr><td><b>"FirmwareName " "
FirmwareNumber;
...
const char  html_control4a[] PROGMEM = "Date/Time: <form method=\"get\" action=\"/control.htm\"><button type=\"submit\" onclick=\"SetDateTime();\">Set</button><input id=\"dm\" type=\"hidden\" name=\"dm\"><input id=\"dd\" type=\"hidden\" name=\"dd\">";
const char  html_control4b[] PROGMEM = "<input id=\"dy\" type=\"hidden\" name=\"dy\"><input id=\"th\" type=\"hidden\" name=\"th\"><input id=\"tm\" type=\"hidden\" name=\"tm\"><input id=\"ts\" type=\"hidden\" name=\"ts\"></form><br />\r\n";
const char  html_control4c[] PROGMEM = "<script>\r\nfunction SetDateTime() {var d = new Date();document.getElementById(\"dd\").value = d.getDate();document.getElementById(\"dm\").value = d.getMonth();document.getElementById(\"dy\").value = d.getFullYear();";
const char  html_control4d[] PROGMEM = "document.getElementById(\"th\").value = d.getHours();document.getElementById(\"tm\").value = d.getMinutes();document.getElementById(\"ts\").value = d.getSeconds();}\r\n</script>\r\n";
const char  html_controlAlign1[] PROGMEM = "Align: <form method=\"get\" action=\"/control.htm\"><button name=\"al\" value=\"1\" type=\"submit\">1 Star</button>";
...
const char  html_controlAlignX[] PROGMEM = "<br /><button name=\"al\" value=\"n\" type=\"submit\">Accept</button></form><br />\r\n";
const char  html_control6[] PROGMEM = "Home/Park: <form method=\"get\" action=\"/control.htm\"><button name=\"hm\" value=\"r\" type=\"submit\">Home (Reset)</button>&nbsp;&nbsp;&nbsp;<button name=\"hm\" value=\"f\" type=\"submit\">Find Home</button><br />";
const char  html_control7[] PROGMEM = "<button name=\"pk\" value=\"p\" type=\"submit\">Park</button>&nbsp;&nbsp;&nbsp;<button name=\"pk\" value=\"u\" type=\"submit\">Un-Park</button><br /><button name=\"pk\" value=\"s\" type=\"submit\">Set-Park</button></form><br />\r\n";
const char  html_control8[] PROGMEM = "Tracking: <form method=\"get\" action=\"/control.htm\"><button name=\"tk\" value=\"on\" type=\"submit\">On</button><button name=\"tk\" value=\"off\" type=\"submit\">Off</button>&nbsp;&nbsp;&nbsp;";
const char  html_control9[] PROGMEM = "<button name=\"tk\" value=\"f\" type=\"submit\">+ (0.1Hz faster)</button><button name=\"tk\" value=\"-\" type=\"submit\">- (0.1Hz slower)</button><button name=\"tk\" value=\"r\" type=\"submit\">Reset (default)</button><br />";
const char  html_control10[] PROGMEM = "<button name=\"tk\" value=\"s\" type=\"submit\">Sidereal</button><button name=\"tk\" value=\"l\" type=\"submit\">Lunar</button><button name=\"tk\" value=\"h\" type=\"submit\">Solar</button>";
#if !defined(MOUNT_TYPE_ALTAZM)
const char  html_control11[] PROGMEM = "</br></br>Compensated Tracking Rate (Pointing Model/Refraction): </br><button name=\"rr\" value=\"otk\" type=\"submit\">Full</button><button name=\"rr\" value=\"on\" type=\"submit\">Refraction Only</button><button name=\"rr\" value=\"off\" type=\"submit\">Off</button>";
const char  html_control12[] = "</br></br>Auto-flip (automatic Meridian flip): </br><button name=\"ac\" value=\"on\" type=\"submit\">On</button><button name=\"ac\" value=\"off\" type=\"submit\">Off</button>";
#endif
const char  html_control13[] = "</form>\r\n";
...
void control_html_page()
{
  ...
  if (MAX_NUM_ALIGN_STARS >= 2)
    if (html_page_step == ++stp) strcpy_P(temp, html_controlAlign2);
  if (MAX_NUM_ALIGN_STARS >= 3)
    if (html_page_step == ++stp) strcpy_P(temp, html_controlAlign3);
  if (MAX_NUM_ALIGN_STARS >= 3)
    if (html_page_step == ++stp) strcpy_P(temp, html_controlAlign3);
  if (MAX_NUM_ALIGN_STARS >= 4)
    if (html_page_step == ++stp) strcpy_P(temp, html_controlAlign4);
  if (MAX_NUM_ALIGN_STARS >= 5)
    if (html_page_step == ++stp) strcpy_P(temp, html_controlAlign5);
  if (MAX_NUM_ALIGN_STARS >= 6)
    if (html_page_step == ++stp) strcpy_P(temp, html_controlAlign6);
  if (MAX_NUM_ALIGN_STARS >= 7)
    if (html_page_step == ++stp) strcpy_P(temp, html_controlAlign7);
  if (MAX_NUM_ALIGN_STARS >= 8)
    if (html_page_step == ++stp) strcpy_P(temp, html_controlAlign8);
  if (MAX_NUM_ALIGN_STARS >= 9)
    if (html_page_step == ++stp) strcpy_P(temp, html_controlAlign9);
...
// The guide.htm page --------------------------------------------------------------------------------
const char  html_guide1[] PROGMEM = "<div class=\"t\"><table width=\"100%\"><tr><td><b>"FirmwareName " "
FirmwareNumber;
...
const char  html_guide4[] PROGMEM = "<script>function guide(dir,start) { var xhttp = new XMLHttpRequest(); xhttp.open('GET', 'guide.txt?dr='+dir+start+'&x='+new Date().getTime(), true);xhttp.send(); }</script>Guide Rate:<br />";
const char  html_guideControls1[] PROGMEM = "<form method=\"get\" action=\"/guide.htm\"><button name=\"gu\" value=\"1\" type=\"submit\">0.5x</button>";
const char  html_guideControls2[] PROGMEM = "<button name=\"gu\" value=\"2\" type=\"submit\">1x</button><button name=\"gu\" value=\"4\" type=\"submit\">Mid</button>";
const char  html_guideControls3[] PROGMEM = "<button name=\"gu\" value=\"6\" type=\"submit\">Fast</button><button name=\"gu\" value=\"8\" type=\"submit\">VFast</button></form><br />Guide:<br />";
const char  html_guideControls4[] PROGMEM = "<div style=\"float: left; border: 2px solid #551111; background-color: #181818; text-align: center; padding: 15px; margin: 5px;\"><button type=\"button\" style=\"width: 60px; height: 50px;\" onmousedown=\"guide('n','1')\" onmouseup=\"guide('n','0')\">North</button><br />";
const char  html_guideControls5[] PROGMEM = "<button type=\"button\" style=\"width: 60px; height: 50px;\" onmousedown=\"guide('e','1')\" onmouseup=\"guide('e','0')\">East</button>";
const char  html_guideControls6[] PROGMEM = "<button type=\"button\" style=\"width: 60px; height: 50px;\" onclick=\"guide('s','y')\">Sync</button><button type=\"button\" style=\"width: 60px; height: 50px;\" onmousedown=\"guide('w','1')\" onmouseup=\"guide('w','0')\">West</button><br />";
const char  html_guideControls7[] PROGMEM = "<button type=\"button\" style=\"width: 60px; height: 50px;\" onmousedown=\"guide('s','1')\" onmouseup=\"guide('s','0')\">South</button></div><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br />\r\n";
...
// The pec.htm page ----------------------------------------------------------------------------------
const char  html_pec1[] PROGMEM = "<div class=\"t\"><table width=\"100%\"><tr><td><b>"FirmwareName " "
FirmwareNumber;
...
const char  html_pecControls1[] PROGMEM = "<form method=\"get\" action=\"/pec.htm\"><button name=\"pe\" value=\"pl\" type=\"submit\">Play</button>";
const char  html_pecControls2[] PROGMEM = "<button name=\"pe\" value=\"st\" type=\"submit\">Stop</button><br /><br /><button name=\"pe\" value=\"cl\" type=\"submit\">Clear</button><button name=\"pe\" value=\"re\" type=\"submit\">Record</button><br />";
const char  html_pecControls3[] PROGMEM = "Clear erases the memory buffer not EEPROM.  During recording corrections are averaged 3:1 favoring the buffer unless the buffer was cleared in which case the full correction is used.<br />";
const char  html_pecControls4[] PROGMEM = "<br /><button name=\"pe\" value=\"wr\" type=\"submit\">Write to EEPROM</button><br />Writes PEC data to EEPROM so it's remembered if OnStep is restarted.  Writing the data can take a few seconds.<br /><br />";
const char  html_pecControls5[] PROGMEM = "</form>\r\n";
...
void pec_html_page()
{
  ...
  if (html_page_step == ++stp)
  {
    ...
    if (pecAutoRecord > 0)
    {
      sprintf(temp3, " (writing to EEPROM, %d bytes left)", pecAutoRecord);
    }
    else
    {
      strcpy(temp3, "");
    ...
    if (pecStatusCh[pecStatus] == 'I')
    {
      strcpy(temp2, "Idle");
    }
    else if (pecStatusCh[pecStatus] == 'p')
    {
      strcpy(temp2, "Play waiting to start");
    }
    else if (pecStatusCh[pecStatus] == 'P')
    {
      strcpy(temp2, "Playing");
    }
    else if (pecStatusCh[pecStatus] == 'r')
    {
      strcpy(temp2, "Record waiting for the index to arrive");
    }
    else if (pecStatusCh[pecStatus] == 'R')
    {
      strcpy(temp2, "Recording");
    }
    else
#endif
      strcpy(temp2, "Unknown");
    strcpy_P(temp1, html_pec5);
    sprintf(temp, temp1, temp2, temp3);
  }
  ...
  if (html_page_step == ++stp)
  {
    if (pecAutoRecord == 0) strcpy_P(temp, html_pecControls4);
  }
...
// The config.htm page --------------------------------------------------------------------------------------
const char  html_config1[] PROGMEM = "<div class=\"t\"><table width=\"100%\"><tr><td><b>"FirmwareName " "
FirmwareNumber;
...
void config_html_page()
{
  ...
  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configMaxRate);
    sprintf(temp, temp1, MaxRate);
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configDegAcc);
    dtostrf(DegreesForAcceleration, 2, 1, temp2);
    sprintf(temp, temp1, temp2);
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configBacklash);
    sprintf(temp, temp1, (long)round(BacklashTakeupRate));
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configStepsDegHA);
    sprintf(temp, temp1, (long)round(StepsPerDegreeAxis1));
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configStepsDegDec);
    sprintf(temp, temp1, (long)round(StepsPerDegreeAxis2));
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configStepsPerSec);
    dtostrf(StepsPerSecondAxis1, 3, 6, temp2);
    sprintf(temp, temp1, temp2);
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configStepsPerWR);
    sprintf(temp, temp1, (long)round(StepsPerWormRotationAxis1));
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configPecBufSize);
    sprintf(temp, temp1, (long)round(PECBufferSize));
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configMPME);
    sprintf(temp, temp1, (long)round(MinutesPastMeridianE));
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configMPMW);
    sprintf(temp, temp1, (long)round(MinutesPastMeridianW));
  }
  ...
  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configMinDec);
    sprintf(temp, temp1, (long)round(MinDec));
  }

  if (html_page_step == ++stp)
  {
    strcpy_P(temp1, html_configMaxDec);
    sprintf(temp, temp1, (long)round(MaxDec));
  }
